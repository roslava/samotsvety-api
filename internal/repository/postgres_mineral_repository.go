...
func (r *PostgresMineralRepository) List(ctx context.Context, filters domain.FilterParams) ([]domain.Mineral, int, error) {
	...
	query := `SELECT slug, scientific, i18n, main_image_url, safety_notes, 
	...
	var rows []mineralRow
	err := r.db.SelectContext(ctx, &rows, query, filters.Limit, offset)
	if err != nil {
		return nil, 0, fmt.Errorf("query error: %w", err)
	}

	var total int
	r.db.GetContext(ctx, &total, "SELECT COUNT(*) FROM minerals")
