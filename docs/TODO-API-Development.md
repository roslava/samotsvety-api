...
- [x] **6.3** Обновить `README.md` проекта
...
## Phase 7: Базовая админка (управление данными) [x]

**7.1** Создать эндпоинты для управления минералами [x]
- POST /api/v1/minerals — создание
- PUT /api/v1/minerals/{slug} — обновление (улучшен merge)
- DELETE /api/v1/minerals/{slug} — удаление

**7.2** Валидация входящих данных [x]
- Использовать go-playground/validator

**7.3** Базовая защита админских эндпоинтов [x]
- Middleware API Key (X-API-Key)
- Только mutating методы защищены

---

## Phase 8: Финализация MVP и полировка

## Phase 8: Финализация MVP и полировка (в работе)

**8.1** Улучшение UpdateMineral (удобный merge + gallery/localities) [x]
**8.2** Качественное наполнение seeds (российские самоцветы) [в работе]
**8.3** Почистить Makefile, улучшить seed-команду [x]
**8.4** Обновить документацию (README + Swagger) [в работе]
