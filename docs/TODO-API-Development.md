...
- [ ] **1.1** Создать `internal/domain/mineral.go`
...
- [ ] **4.1** Расширенная фильтрация и сортировка в List
...
- [ ] **4.3** Эндпоинт `GET /api/v1/filters`
...
- [ ] **5.1** Создать систему загрузки данных
  - `seeds/` папка с JSON-файлами минералов
  - Скрипт / команда `make seed` или Go-утилита для импорта

- [ ] **5.2** Подготовить и импортировать первые 10–15 минералов
